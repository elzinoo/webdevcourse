 ...
 * doctoral track, by interacting with the API. Users are 
...
function Doctoral() {
    ...
    const track = (value) => (value.ShortNameOfTrack === "doctoral");
    
    const showDoctoral = papers
        .filter(track)
        .filter(searchPapers)
        .filter(selectPapers)
        .map((value, key) => (
